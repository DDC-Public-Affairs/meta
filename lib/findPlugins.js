...
function getDefaultGlobalRoot() {
  var globalRoot = (cp.execSync('npm root -g') + '').trim();
  return escapeParenthesisInString(globalRoot);
}

function escapeParenthesisInString(escapee) {
  return escapee.replace(/(?=[() ])/g, '\\');
}
